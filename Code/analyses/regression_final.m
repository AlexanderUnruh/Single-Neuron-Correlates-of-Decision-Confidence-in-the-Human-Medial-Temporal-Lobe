function regression_final(windowtype, modeltypeIn, typeCorrIn, extraname, windowsize, stepsize, inputRegion2, autoregresive_corr, alignedtoDecision)





% This is the main analysis code for the correlations and regressions used
% in the papaer
...
if ismember(typeCorrIn, {'Spearman', 'Pearson', 'Kendall'})
    typeCorr=typeCorrIn;
else
...
satiationfolder = '/media/raid/Alex/Reclustered analysis All/';
cd(satiationfolder);

if autoregresive_corr == 0
    ana_title= ['mixed_model',num2str(modeltypeIn), '_',typeCorrIn,'_window',num2str(windowtype),'_kernel', num2str(windowsize), 'ms_step' , num2str(stepsize), 'ms_',extraname];
elseif autoregresive_corr == 1
    ana_title= ['mixed_model',num2str(modeltypeIn), '_',typeCorrIn,'_window',num2str(windowtype),'_kernel', num2str(windowsize), 'ms_step' , num2str(stepsize), 'ms_',extraname, '_autocorr'];
end
...
disp(['Folder to save the dat is:', folder_to_save])
...
for hhh=1:length(inputRegion2)
    ...
    for sess = 1:size(outfiles,1)
        ...
        %assert(isequal(Consumedindex, ConsumedindexN))
        ...
        for i= 1:20
            for hh=1:20
                if i ~=hh
                    ...
                    invval2_C_Rat(indPair1,1) =  val_C_Rat(indPair2,2);  % the other inverse chosen and unchosen are based in the stimulus value before and after,now it is based
                    invval2_C_Rat(indPair2,2) =  val_C_Rat(indPair1,1);  %on the value of the chosen value in the trial type, so if the stimulus chosen changes, it is the value of
                    invval2_C_Rank(indPair1,1) =  val_C_Rank(indPair2,2); % the other stimulus when chosen.
        ...
        %% define reaction-times-Rased windows
        ...
        for unit = 1:n_ses_units
            ...
            if windowtype == 10  % gaussian kernel
                ...
                [mfrOKResp1] = kernelgauss(likAFCOK1, windowsize, resolution, stepsize, newwinwosreg, stdkernel);
                [mfrOKResp2] = kernelgauss(likAFCOK2, windowsize, resolution, stepsize, newwinwosreg, stdkernel);
                [mfrOKRatResp1] = kernelgauss(ratOKLik1, windowsize, resolution, stepsize, newwinwosregRating, stdkernel);
                [mfrOKRatResp2] = kernelgauss(ratOKLik2, windowsize, resolution, stepsize, newwinwosregRating, stdkernel);
                ...
                % create meanmfr for Valuation task, since for comparison
                % of the change in mfr and change in a variable, an average
                % has to be computed for the valuation task. This is
                % because each item was shown three times and no sirect
                % compaison could be used, se we average on bases of the 20
                % stimuli
